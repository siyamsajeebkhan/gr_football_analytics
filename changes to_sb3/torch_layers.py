...
def create_mlp(
    ...
    if len(net_arch) > 0:
        ...
        # My change
        # With BN
        # modules = [nn.Linear(input_dim, net_arch[0]), nn.BatchNorm1d(net_arch[0]), activation_fn(), nn.Dropout(0.25)]

        # Without BN
        modules = [nn.Linear(input_dim, net_arch[0]), activation_fn(), nn.Dropout(0.25)]
    else:
        modules = []
    ...
    def __init__(
        ...
        for layer in net_arch:
            if isinstance(layer, int):  # Check that this is a shared layer
                ...
                print("If you don't want to use dropout, just comment line 198 in torch_layers.py file")
                shared_net.append(nn.Dropout(0.25))
